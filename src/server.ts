...
app.get("/schedule/confidence", async (req, res) => {
  ...
  res.json({
    ...
    runs: history.map((run) => ({
      ...
      quarantined_count: run.quarantined_row_count,
      current_run_quarantined_count: run.current_run_quarantined_row_count,
      current_run_conflict_count: run.current_run_conflict_row_count,
      retained_prior_review_count: run.retained_prior_review_row_count,
      review_reason_counts: run.review_reason_counts_json ? JSON.parse(run.review_reason_counts_json) : undefined,
      partial_reason: run.partial_reason,
      artifact_retrieval_enabled: run.artifact_retrieval_enabled === 1,
