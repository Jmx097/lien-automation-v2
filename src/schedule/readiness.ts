...
import {
  getSiteComplianceState,
  type SiteComplianceSummary,
} from "../scheduler";
import {
  ScheduledRunStore,
  getSchedulerStoreReadiness,
} from "../scheduler/store";
import {
  createDefaultConnectivityState,
  getNextAllowedRunAt,
  type SiteConnectivityStatus,
...
function checkRequiredEnv(): ReadinessCheck {
  const missing = ["SHEET_ID", "SHEETS_KEY", "SCHEDULE_RUN_TOKEN"].filter(
  ...
  if (missing.length > 0 || !hasBrowserTransport) {
    ...
    if (!hasBrowserTransport) {
      details.push(
        "one of BRIGHTDATA_BROWSER_WS, BRIGHTDATA_PROXY_SERVER, SBR_CDP_URL",
      );
    }
    return {
...
function checkSiteScheduleConfig(): ReadinessCheck {
  ...
  const missing = requiredSiteScheduleVars.filter(
    (envVar) => !process.env[envVar],
  );
...
function checkDownstreamCredentialsLoaded(): ReadinessCheck {
  ...
  try {
    const parsed = JSON.parse(sheetsKeyRaw.replace(/^'+|'+$/g, "")) as Record<
      string,
      unknown
    >;
    const hasClientEmail =
      typeof parsed.client_email === "string" && parsed.client_email.length > 0;
    const hasPrivateKey =
      typeof parsed.private_key === "string" && parsed.private_key.length > 0;
...
function checkOCRReady(): ReadinessCheck {
  ...
  if (!result.ok) {
    return {
      ...
      ok: false,
      detail:
        result.detail ?? `Missing OCR binaries: ${result.missing.join(", ")}`,
    };
  }
...
export function checkSiteComplianceHealthy(
  siteCompliance: Record<SupportedSite, SiteComplianceSummary>,
): ReadinessCheck {
  const failingSites = Object.entries(siteCompliance)
    .filter(([, summary]) => summary.rolling_sla_status === "breached")
    .map(
      ([site, summary]) =>
  ...
  const cadenceWarningSites = Object.entries(siteCompliance)
    .filter(
      ([, summary]) =>
        summary.observed_run_count > 0 &&
        !summary.previous_business_day_slots_ok,
    )
    .map(
      ([site, summary]) =>
        `${site}: previous_day_slots=${summary.previous_business_day_slot_success_count}/${summary.previous_business_day_expected_slots}`,
    );
  const warmingSites = Object.entries(siteCompliance)
    .filter(([, summary]) => summary.rolling_sla_status === "insufficient_data")
    .map(([site]) => site);
  const details = [
    cadenceWarningSites.length > 0
      ? `Cadence history warning (not readiness-blocking): ${cadenceWarningSites.join("; ")}`
      : undefined,
    warmingSites.length > 0
...
export async function getScheduleReadinessReport(): Promise<ScheduleReadinessReport> {
  ...
  const dbCheck: ReadinessCheck = storeReadiness.ok
    ? {
        name: "db_reachable",
        ok: true,
        detail: `scheduler_store=${storeReadiness.backend}`,
      }
        ...
        try {
          return await Promise.all(
            supportedSites.map(async (site) => {
              const state =
                (await store.getConnectivityState(site)) ??
                createDefaultConnectivityState(site);
              return [
                site,
                {
                  status: state.status,
  ...
  if (
    maricopaPersistedState.artifactRetrievalEnabled &&
  ...
  return {
    ...
    checks,
    merged_output: {
      source_spreadsheet_id_suffix:
        mergedConfig.source_spreadsheet_id.slice(-6),
      target_spreadsheet_id_suffix:
        mergedConfig.target_spreadsheet_id.slice(-6),
      target_reachable: targetAccess.ok,
      fallback_active:
        !targetAccess.ok &&
        mergedConfig.target_spreadsheet_id !==
          mergedConfig.source_spreadsheet_id,
    ...
    maricopa: {
      artifact_retrieval_enabled:
        maricopaPersistedState.artifactRetrievalEnabled,
      enrichment_mode: maricopaPersistedState.artifactRetrievalEnabled
        ? "artifact_enriched"
        : "api_only",
      production_ready_for_enriched_runs:
        maricopaPersistedState.artifactRetrievalEnabled &&
        !maricopaPersistedState.refreshRequired,
      ...
      artifact_candidates_present:
        maricopaPersistedState.artifactCandidatesPresent,
      ...
      artifact_candidates_updated_at:
        maricopaPersistedState.artifactCandidatesUpdatedAt,
      artifact_candidate_age_minutes:
        maricopaPersistedState.artifactCandidateAgeMinutes,
      artifact_candidate_max_age_minutes:
        maricopaPersistedState.artifactCandidateMaxAgeMinutes,
